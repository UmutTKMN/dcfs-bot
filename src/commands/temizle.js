...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const menu = new StringSelectMenuBuilder()
      ...
      .addOptions([
        ...
        {
          label: "Bu kanaldaki her şey",
          value: "all",
          description: "Kanalda silinebilen tüm mesajlar",
        },
      ]);
    const row = new ActionRowBuilder().addComponents(menu);
    await interaction.reply({
      content: "Kaç mesaj silinsin?",
      components: [row],
      flags: 64,
    });
  ...
  async handleSelect(interaction) {
    if (!interaction.member.permissions.has("ManageMessages")) {
      return await interaction.reply({
        content:
          "Bu işlemi yapmak için mesajları yönet iznine sahip olmalısın.",
        flags: 64,
      });
    }
    ...
    try {
      if (value === "all") {
        ...
        await interaction.update({
          content: `Kanalda toplam ${deletedCount} mesaj silindi.`,
          components: [],
        });
      } else {
