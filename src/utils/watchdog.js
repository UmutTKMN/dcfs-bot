...
const CONFIG = {
  ...
  CHECK_INTERVAL_MS: getConfig.number(
    "FS25_BOT_WATCHDOG_CHECK_INTERVAL_MS",
    60000
  ), // Her dakika kontrol et
  RESTART_DELAY_MS: getConfig.number(
    "FS25_BOT_WATCHDOG_RESTART_DELAY_MS",
    5000
  ), // Yeniden başlatmadan önce 5 saniye bekle
  MAX_CRASHES: getConfig.number("FS25_BOT_WATCHDOG_MAX_CRASHES", 10), // Vazgeçmeden önce maksimum çökme sayısı
  CRASH_RESET_TIME_MS: getConfig.number(
    "FS25_BOT_WATCHDOG_CRASH_RESET_TIME_MS",
    3600000
  ), // 1 saat stabil çalıştıktan sonra çökme sayacını sıfırla
  LOG_DIR: getConfig.string(
    "FS25_BOT_LOG_DIR",
...
const logFile = path.join(
  CONFIG.LOG_DIR,
  `watchdog-${new Date().toISOString().replace(/:/g, "-")}.log`
);
let logStream;
  ...
  try {
    ...
    botProcess.on("exit", (code, signal) => {
      ...
      if (crashes < CONFIG.MAX_CRASHES) {
        log(
          `Yeniden başlatmadan önce ${CONFIG.RESTART_DELAY_MS}ms bekleniyor...`
        );
        setTimeout(startBot, CONFIG.RESTART_DELAY_MS);
      } else {
        log(
          "Çok fazla çökme. Vazgeçiliyor. Lütfen loglarınızı kontrol edin ve manuel olarak yeniden başlatın."
        );
      }
    });
  } catch (error) {
